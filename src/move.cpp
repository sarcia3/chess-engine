...
board::move board::move_from_pair(pair<int, int> arg) const {
    ...
    if(res.start_pos == res.end_pos) {
        if(res.start_pos < 2)  {
            if(castle[0]) res.castle_disruptions[0] = true;
            if(castle[1]) res.castle_disruptions[1] = true;
        }
        else {
            if(castle[2]) res.castle_disruptions[2] = true;
            if(castle[3]) res.castle_disruptions[3] = true;
